// ==UserScript==
// @name         短视频审核-一页勾选版【试运行】
// @namespace    https://doubao.com/userscripts
// @version      1.0.0-trial
// @description  回到"勾选"模式：逐页扫描 → YOLO检测图片有无人物 → 错误数据(无人)勾选后批量错误 → 剩下的批量正确 → 等下一页刷新。默认预演模式(只看不点)
// @author       Doubao
...
// 处理逻辑（一页一轮）：
//   1) 等表格稳定，取当前页所有行（约100条），同时取出第5列图、第6列图
//   2) 得分 ≤ scoreWrongMax(默认74) → 直接判【错误】（不送检测，省时间）
//      得分 ≥ 75 → 送后端：
//         a) 明细检测：有人物(人脸/背影/侧脸/全身) 且 未检出手机/屏幕
//         b) 相似度比对：第5列 vs 第6列（SSIM+dHash+直方图），≥ 阈值(默认0.60)
//         两条都过 → 正确；无人 / 手机 / 屏幕 / 相似度低 / 检测失败 → 错误
//   3) 三种运行模式（面板切换）：
//        scan  = 只扫描，不勾选、不提交（「只扫当前页」按钮）
//        check = ★默认：自动勾选【错误】行，但不点「批量错误/批量正确」（测试模式）
//        full  = 正式：勾错误→批量错误→等错误行消失→勾正确→批量正确→等下一页刷新
//   4) 一页处理完等系统刷新出新的一页，继续下一轮，直到无新数据。

(function () {
  'use strict';

  console.log('[短视频勾选-试运行] 已加载');
  ...
  function safeClick(el) {
    ...
    try {
      ...
      console.warn('[勾选试运行] 点击失败:', e.message || e);
      return false;
    }
  }
  ...
  async function waitNextPage(beforeFirstKey, timeout) {
    ...
    while (Date.now() - start < timeout) {
      ...
      if (count === 0) return 0;
      if (firstKey && firstKey !== beforeFirstKey) {
  ...
  async function processOnePage() {
    ...
    if (wrong.length > 0) {
      ...
      if (!gone) {
        log('等待错误行消失超时（' + Math.round(CONFIG.refreshWaitMax / 1000) + 's），继续尝试批量正确');
      } else {
        log('✅ 错误行已全部从列表消失');
      }
      setStatus('等待系统处理...');
      await delay(CONFIG.afterSubmitWait);
      if (!state.running) return 'stopped';
    }
  ...
  function buildPanel() {
    ...
    panel.id = 'v1-panel';
    panel.innerHTML = `
      <div class="v1-title" id="v1-title">短视频勾选助手（试运行）
      ...
      <div class="v1-main" id="v1-main">
        ...
        <div class="v1-tip">
          规则：<b>得分 ≤ ${CONFIG.scoreWrongMax}</b> → 直接错误；得分更高 → 图片检测<br>
          检测：有人(置信度≥${CONFIG.personConf}) 且 无手机/屏幕(置信度≥${CONFIG.objectConf}) 且 相似度≥${CONFIG.simThreshold} → 正确<br>
          无人 / 手机 / 屏幕 / 相似度低 / 检测失败 → 错误<br>
          <b>测试模式(默认开)</b>：只自动勾选错误行，不点批量错误/批量正确；确认无误后取消勾选再正式跑<br>
          需先启动本地后端 127.0.0.1:5002
        </div>
    ...
    function readPanelConfig() {
      ...
      if (isNaN(CONFIG.simThreshold)) CONFIG.simThreshold = 0.45;
  ...
  function startRun() {
    if (state.running) return;
    state.running = true;
    state.mode = CONFIG.testMode ? 'check' : 'full';
    state.rounds = 0;
    state.wrongCount = 0;
