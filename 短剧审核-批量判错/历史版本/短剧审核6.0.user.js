// ==UserScript==
// @name         Shift短剧审核-双模式全剧审核版【6.0】
// @namespace    https://doubao.com/userscripts
// @version      6.1.0
// @description  两种处理方式：①匹配分>阈值 → 不展开合集直接全剧审核；②其余(匹配分≤阈值/命中关键词/短剧名≤4字且与第4列原短剧名不相似)视为全错 → 展开+全选+批量错误(等全部594)+全剧审核；含慢处理与问题合集跳过
// @author       Doubao
...
// 6.1.0 整合记录：
//   1) 【修 bug】按钮查找不再退化成全表格搜索：新增 findRowButton()，严格限定「本行 → 本行展开区 → 几何上属于本行的按钮」，
//      避免处理第 N 行时误点到其它合集的全剧审核按钮；并限定在本行所在 tbody 内（排除分页按钮）。
//   2) 【防重复提交】按钮消失时视为已提交（existedAtStart 判定），避免确认弹窗未捕获到就重复点击「全剧审核」。
//   3) 【清理死代码】删除未使用的 isVisible()、state.statusText、stats.judged、similar 的 origLen。
//   4) 保持原有判定流程与等待策略不变。
...
(function () {
  ...
  function log(msg) {
    console.log('[短剧6.0] ' + msg);
    const el = document.querySelector('#s6-log');
    if (!el) return;
  ...
  function setStatus(msg) {
    console.log('[状态] ' + msg);
    const el = document.querySelector('#s6-status');
    if (el) el.textContent = msg;
    // 最小化时下方也显示当前状态
    const t = document.querySelector('#s6-mini-status');
  ...
  function safeClick(el) {
    ...
    catch (e) { console.warn('[短剧6.0] 点击失败:', e.message || e); return false; }
  }
  ...
  // ========== 等待全部子样本变成 594（判错完成） ==========
  // 模式2 的"等响应"：批量错误提交后，必须所有子样本状态码都变成 594 才算成功
  // （不能只看"不再是0"，否则子表里原本就有 777/601 时会误判为已完成，导致过早点全剧审核）
  async function waitAllJudged(expandedRow, timeout) {
    ...
    while (Date.now() - start < timeout) {
      ...
      if (stats.total > 0 && stats.wrong === stats.total) return true;
      await delay(CONFIG.checkInterval);
    }
    return false;
  }
  ...
  async function processOneCollection(R, meta) {
    ...
    if (!ok) {
      ...
      log('等待594超时(' + Math.round(judgeWait / 1000) + 's)，状态码分布: ' + last.detail + '，判定该合集有问题');
      return { result: 'problem', mode: 2 };
    }
    log('✅ 判错完成，全部子样本状态码=' + CONFIG.wrongCode);
  ...
  function updatePanel() {
    const c = document.querySelector('#s6-count');
    if (c) c.textContent = state.processedCount;
    const r = document.querySelector('#s6-row');
    if (r) r.textContent = (state.workIndex + 1);
    const m1 = document.querySelector('#s6-m1');
    if (m1) m1.textContent = state.mode1Count;
    const m2 = document.querySelector('#s6-m2');
  ...
  function buildPanel() {
    if (document.querySelector('#s6-panel')) return;
    const panel = document.createElement('div');
    panel.id = 's6-panel';
    panel.innerHTML = `
      <div class="p6-title" id="s6-title">短剧审核助手 6.0
        <button id="s6-min" class="p6-min" title="最小化/展开">—</button>
      </div>
      <div class="p6-statusline"><span id="s6-status">空闲</span></div>
      <div class="p6-main" id="s6-main">
        <div class="form-row">
          <label>起始行号：</label>
        ...
        <div class="stat-row">当前行：第 <b id="s6-row">1</b> 行 &nbsp;|&nbsp; 已完成：<b id="s6-count">0</b> 个</div>
        <div class="stat-row">模式1(直接全剧)：<b id="s6-m1">0</b> &nbsp;|&nbsp; 模式2(全选判错)：<b id="s6-m2">0</b></div>
        <div class="btn-row">
        ...
        <div class="p6-log" id="s6-log"></div>
      </div>
      <div class="p6-mini-status" id="s6-mini-status"></div>
    `;
    document.body.appendChild(panel);

    const css = document.createElement('style');
    css.textContent = `
      #s6-panel{position:fixed;top:100px;right:20px;width:300px;background:#fff;border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,0.15);z-index:999999;font-size:13px;font-family:system-ui,sans-serif;}
      .p6-title{padding:8px 12px;background:#722ed1;color:#fff;border-radius:8px 8px 0 0;font-weight:600;position:relative;}
      .p6-min{position:absolute;right:8px;top:6px;width:22px;height:22px;line-height:18px;border:none;border-radius:4px;background:rgba(255,255,255,.25);color:#fff;cursor:pointer;font-size:14px;}
      .p6-min:hover{background:rgba(255,255,255,.45);}
      .p6-statusline{padding:6px 12px;background:#fff7e6;border-bottom:1px solid #ffe7ba;color:#d46b08;font-size:12px;min-height:26px;}
      .p6-statusline span{word-break:break-all;}
      .p6-mini-status{display:none;padding:6px 12px;background:#fff7e6;color:#d46b08;font-size:12px;border-radius:0 0 8px 8px;}
      .p6-main{padding:10px 12px;}
      ...
      .tip b{color:#722ed1;}
      .p6-log{max-height:160px;overflow-y:auto;background:#fafafa;border-radius:4px;padding:5px 7px;font-size:11px;font-family:Consolas,monospace;line-height:1.6;color:#666;word-break:break-all;}
    `;
    document.head.appendChild(css);

    initDrag(panel, panel.querySelector('#s6-title'));

    // 最小化/展开
    const main = panel.querySelector('#s6-main');
    const miniStatus = panel.querySelector('#s6-mini-status');
    document.querySelector('#s6-min').addEventListener('click', () => {
      const hidden = main.style.display === 'none';
      main.style.display = hidden ? '' : 'none';
      miniStatus.style.display = hidden ? 'none' : '';
      document.querySelector('#s6-min').textContent = hidden ? '—' : '□';
    });
