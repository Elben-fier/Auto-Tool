// ==UserScript==
// @name         Shift短剧审核-自动批量判错工具【通用弹窗定位版4.3】
// @namespace    https://doubao.com/userscripts
// @version      4.3.0
// @description  增加起始行号选择+批量错误后2s等待
// @author       Doubao
...
(function () {
  ...
  const processOneDrama = async () => {
    ...
    console.log(`===== 处理 第${CONFIG.startRow}行 [匹配最高分: ${parentScore}] =====`);
  ...
  const renderPanel = () => {
    ...
    panel.innerHTML = `
      <div class="panel-title">短剧审核助手 4.3</div>
      <div class="panel-body">
        ...
        <div class="tip">父级≤阈值: 全选→批量错误→全剧审核<br>父级>阈值: Shift多选低分→批量错误→全剧审核<br>批量错误后等待2s再审核</div>
      </div>
    `;
