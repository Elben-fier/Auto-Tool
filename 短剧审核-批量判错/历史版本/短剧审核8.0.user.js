// ==UserScript==
// @name         Shift短剧审核-智能判定全剧审核版【8.0】
// @namespace    https://doubao.com/userscripts
// @version      8.0.0
// @description  智能判定：关键词双列比对 + 相似度闸门 + 匹配分/子样本分档 → 决定【直接全剧审核】或【全选+批量错误+全剧审核】；含30s/60s重试、永久跳过与严格等待加载
// @author       Doubao
...
// ============================== 8.0.0 更新记录 ==============================
//  【一】关键词规则改为"双列比对"：
//        第3列（短剧名）命中关键词 → 再看第4列（疑似抄袭原短剧名）是否也含【同一个词】：
//          两边都有 → 视为【疑似正确】→ 继续后面的判定（不再直接判错）
//          第4列没有 → 直接模式2（全错）
//  【二】等待/重试逻辑重写：
//        · 全剧审核是否完成 → 只用第2列【合集id】（唯一）判断，不再用文本兜底（避免文本重复卡死）
//        · 批量错误仍看子样本 594 状态
//        · 等待期间第 30s / 60s 各重试点击一次同一按钮（每个按钮最多点 3 次=首次+30s+60s）
//          仅当"页面已静止（无转圈）但状态没变化"时才重试（还在转圈说明系统在处理，继续等）
//        · 超过 60s 重试仍无响应 → 【永久跳过该合集，换下一行继续审核】（不再整脚本停止）
//  【三】匹配分分档（母本第5列）：
//        > 95            → 模式1（直接全剧审核，不展开）
//        90 ≤ 分 ≤ 95    → 疑似正确 → 展开后判断：子样本得分≥90 的条数 ≥3 或 存在状态码 601/777
//                          → 模式1（全剧审核）；否则 → 模式2（全选判错）
//        < 90            → 模式2（直接全错，不展开）
//        读不到          → 展开看 601/777 痕迹兜底
//  【保留】7.0 的严格等待加载（转圈闸门）、相似度闸门（短剧名≤4字 vs 第4列）
// ==========================================================================
...
(function () {
  ...
  function log(msg) {
    console.log('[短剧8.0] ' + msg);
    const el = document.querySelector('#s8-log');
    if (!el) return;
  ...
  function setStatus(msg) {
    console.log('[状态] ' + msg);
    const el = document.querySelector('#s8-status');
    if (el) el.textContent = msg;
    // 最小化时下方也显示当前状态
    const t = document.querySelector('#s8-mini-status');
  ...
  function safeClick(el) {
    ...
    catch (e) { console.warn('[短剧8.0] 点击失败:', e.message || e); return false; }
  }
  ...
  function isLoading() {
    ...
    for (const sp of document.querySelectorAll('.ant-spin-spinning')) {
      if (!sp.closest('#s8-panel')) return true;
    }
    return false;
  }
  ...
  async function processOneCollection(R, meta) {
    ...
    log('相似度比对：短剧名(' + sim.nameLen + '字) → ' +
      (gateActive ? (sim.similar ? '相似·疑似正确' : '不相似') : '跳过闸门(名称>' + CONFIG.similarGateNameLen + '字)') +
      '（' + sim.detail + '）');
  ...
  function updatePanel() {
    const c = document.querySelector('#s8-count');
    if (c) c.textContent = state.processedCount;
    const r = document.querySelector('#s8-row');
    if (r) r.textContent = (state.workIndex + 1);
    const m1 = document.querySelector('#s8-m1');
    if (m1) m1.textContent = state.mode1Count;
    const m2 = document.querySelector('#s8-m2');
    if (m2) m2.textContent = state.mode2Count;
    const sk = document.querySelector('#s8-skip');
  ...
  function buildPanel() {
    if (document.querySelector('#s8-panel')) return;
    const panel = document.createElement('div');
    panel.id = 's8-panel';
    panel.innerHTML = `
      <div class="p8-title" id="s8-title">短剧审核助手 8.0
        <button id="s8-min" class="p8-min" title="最小化/展开">—</button>
      </div>
      <div class="p8-statusline"><span id="s8-status">空闲</span></div>
      <div class="p8-main" id="s8-main">
        <div class="form-row">
          <label>起始行号：</label>
        ...
        <div class="form-row">
          <label class="p8-check"><input type="checkbox" id="strict-loading" ${CONFIG.strictLoading ? 'checked' : ''}> <b>严格等待加载完成</b></label>
          <span>转圈消失才点下一步</span>
        </div>
        <div class="stat-row">当前行：第 <b id="s8-row">1</b> 行 &nbsp;|&nbsp; 已完成：<b id="s8-count">0</b> 个 &nbsp;|&nbsp; 已跳过：<b id="s8-skip">0</b> 个</div>
        <div class="stat-row">模式1(直接全剧)：<b id="s8-m1">0</b> &nbsp;|&nbsp; 模式2(全选判错)：<b id="s8-m2">0</b></div>
        <div class="btn-row">
        ...
        <div class="tip">
          <b>8.0 判定</b>：① 关键词"双列比对"：第3列命中关键词且<b>第4列也有同一个词</b> → 疑似正确（继续）；第4列没有 → 直接模式2<br>
          ② 相似度闸门（仅短剧名 ≤${CONFIG.similarGateNameLen} 字时）：不相似 → 模式2<br>
          ...
          <b>8.0 等待/重试</b>：全剧审核是否完成<b>只看第2列合集id</b>（唯一）；等待期间第 ${CONFIG.retryAtSec.join('s / ')}s 各重试点击一次同一按钮（最多3次）；仍无响应 → <b>永久跳过该合集，换下一行继续</b><br>
          <b>严格等待</b>：每次点击前/展开后/提交后都等 antd 转圈消失；转圈期间不判定完成
        </div>
        <div class="p8-log" id="s8-log"></div>
      </div>
      <div class="p8-mini-status" id="s8-mini-status"></div>
    `;
    document.body.appendChild(panel);

    const css = document.createElement('style');
    css.textContent = `
      #s8-panel{position:fixed;top:100px;right:20px;width:300px;background:#fff;border-radius:8px;box-shadow:0 4px 16px rgba(0,0,0,0.15);z-index:999999;font-size:13px;font-family:system-ui,sans-serif;}
      .p8-title{padding:8px 12px;background:#722ed1;color:#fff;border-radius:8px 8px 0 0;font-weight:600;position:relative;}
      .p8-min{position:absolute;right:8px;top:6px;width:22px;height:22px;line-height:18px;border:none;border-radius:4px;background:rgba(255,255,255,.25);color:#fff;cursor:pointer;font-size:14px;}
      .p8-min:hover{background:rgba(255,255,255,.45);}
      .p8-statusline{padding:6px 12px;background:#fff7e6;border-bottom:1px solid #ffe7ba;color:#d46b08;font-size:12px;min-height:26px;}
      .p8-statusline span{word-break:break-all;}
      .p8-mini-status{display:none;padding:6px 12px;background:#fff7e6;color:#d46b08;font-size:12px;border-radius:0 0 8px 8px;}
      .p8-main{padding:10px 12px;}
      ...
      .p8-check{display:flex;align-items:center;gap:4px;cursor:pointer;color:#d46b08;font-weight:600;}
      ...
      .tip b{color:#722ed1;}
      .p8-log{max-height:160px;overflow-y:auto;background:#fafafa;border-radius:4px;padding:5px 7px;font-size:11px;font-family:Consolas,monospace;line-height:1.6;color:#666;word-break:break-all;}
    `;
    document.head.appendChild(css);

    initDrag(panel, panel.querySelector('#s8-title'));

    // 最小化/展开
    const main = panel.querySelector('#s8-main');
    const miniStatus = panel.querySelector('#s8-mini-status');
    document.querySelector('#s8-min').addEventListener('click', () => {
      const hidden = main.style.display === 'none';
      main.style.display = hidden ? '' : 'none';
      miniStatus.style.display = hidden ? 'none' : '';
      document.querySelector('#s8-min').textContent = hidden ? '—' : '□';
    });
  ...
  function startRun() {
    ...
    log('判定：①关键词双列比对(第3列命中且第4列同词→疑似正确；否则全错) ②相似度闸门(短剧名≤' +
      CONFIG.similarGateNameLen + '字) ③匹配分>' + CONFIG.parentScoreThreshold + '→模式1 ④[' +
